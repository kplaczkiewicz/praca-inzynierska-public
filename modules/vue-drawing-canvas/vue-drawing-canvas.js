...
var VueDrawingCanvas = /*#__PURE__*/defineComponent({
    ...
    watch: {
        ...
        additionalImage: function () {
            this.loadedAdditionalImage = null;
        }
    },
    methods: {
        async setContext() {
        ...
        async setBackground() {
            ...
            await this.$nextTick(async () => {
                ...
                await this.drawAdditionalImage();
            });
            this.save();
        },
        ...
        async drawBackgroundImage() {
            if (!this.loadedImage) {
                ...
                this.drawBackgroundPattern(this.loadedImage);
            }
        },

        drawBackgroundPattern(image) {
            const repeatX = Math.ceil(this.width / image.width);
            const repeatY = Math.ceil(this.height / image.height);
        ...
        async drawAdditionalImage() {
            if (!this.additionalImage) return;

            if (!this.loadedAdditionalImage) {
                return new Promise(resolve => {
                    ...
                    const image = new Image();
                    image.src = this.additionalImage.src;
                    image.crossOrigin = "anonymous";
                    
                    image.onload = () => {
                        this.context.drawImage(image, this.additionalImage.x, this.additionalImage.y);
                        this.loadedAdditionalImage = image;
                        resolve();
                    };
                });
            } else {
                this.context.drawImage(this.loadedAdditionalImage, this.additionalImage.x, this.additionalImage.y);
            }
        },
